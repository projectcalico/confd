...
confd -confdir=/etc/calico/confd -onetime ${ETCD_ENDPOINTS_CONFD} \
        ...
        -client-ca-keys=${ETCD_CA_CERT_FILE} >/dev/null 2>&1 || true
confd -confdir=/etc/calico/confd -onetime ${ETCD_ENDPOINTS_CONFD} \
        -client-key=${ETCD_KEY_FILE} -client-cert=${ETCD_CERT_FILE} \
        -client-ca-keys=${ETCD_CA_CERT_FILE} >/dev/null 2>&1 || true
