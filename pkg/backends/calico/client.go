...
func (c *client) getPrefixAdvertisementsKVPair(v3res *apiv3.BGPConfiguration, key interface{}) {
	...
	if v3res != nil && v3res.Spec.PrefixAdvertisements != nil {
		...
		for _, prefixAdvertisement := range v3res.Spec.PrefixAdvertisements {
			...
			// Storing community in map as key to avoid duplicates
			communitiesMap := make(map[string]struct{})
				...
				if !isCommunity {
					for _, definedCommunity := range definedCommunities {
						if definedCommunity.Name == c {
							communitiesMap[definedCommunity.Value] = struct{}{}
							break
						}
					}
				} else {
					communitiesMap[c] = struct{}{}
				}
			}
			...
			if strings.Contains(cidr, ":") {
				ipv6PrefixToAdvertise = append(ipv6PrefixToAdvertise, bgpPrefix{
					CIDR:        cidr,
					Communities: getCommunitiesArray(communitiesMap),
				})
			} else {
...
func getCommunitiesArray(communitiesMap map[string]struct{}) []string {
	var communityValue []string
	for k := range communitiesMap {
		communityValue = append(communityValue, k)
	}
